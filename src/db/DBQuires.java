...
public class DBQuires {
	...
	Connection conn=dao.getConnection();
	String insertquery="insert into imageinfo2 values(?,?,?,?)";
	PreparedStatement pstmt=null;
	try{
		pstmt=conn.prepareStatement(insertquery);;
...
System.out.println(preparedStatement.toString());
	...
	public String checkSender(String imageName) {
		...
		DAO data=new DAO();
		Connection conn=data.getConnection();
		String validateUser = "select * from  imageinfo2 where imagename=?";
		PreparedStatement preparedStatement;
		try {
			preparedStatement = conn.prepareStatement(validateUser);
...
System.out.println(preparedStatement.toString());
	...
	public ArrayList<String> getDownloading(String emailid) {
		...
		DAO data=new DAO();
		Connection conn=data.getConnection();
		String validateUser = "select * from  imageInfo2 where name=? ";
		PreparedStatement preparedStatement;
		try {
			preparedStatement = conn.prepareStatement(validateUser);
			preparedStatement.setString(1, emailid);
...
System.out.println(preparedStatement.toString());
	...
	public void updateCount(String actualimage) {
		...
		DAO data=new DAO();
		Connection conn=data.getConnection();
		String validateUser = "select recievedqty from  imageinfo2 where imagename=?";
		PreparedStatement preparedStatement;
		try {
			preparedStatement = conn.prepareStatement(validateUser);
			...
			if(result.next())
			{
				...
				validateUser = "update imageinfo2 set recievedqty=? where imagename=?";
				preparedStatement = conn.prepareStatement(validateUser);
...
System.out.println(preparedStatement.toString());
	...
	public int checkMatch(String actualimage) {
		...
		DAO data=new DAO();
		Connection conn=data.getConnection();
		String validateUser = "select qty,recievedqty from  imageInfo2 where imagename=? ";
		PreparedStatement preparedStatement;
		try {
			preparedStatement = conn.prepareStatement(validateUser);
			preparedStatement.setString(1, actualimage);
